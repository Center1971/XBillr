...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::LineCounter;
...
;
    ...
    unless (exists $cache->{$from})
    {
        $parser->{lastlinenum} = $parser->{offsetlinenum}
          - Config::File::RRJSON::ParserRuntime::_linecount(substr($parser->{fulltext},$from))
          + 1;
...
sub resync   # ($linecounter)
{
    ...
    die "Tried to alter something other than a LineCounter\n"
        unless $self =~ /Config::File::RRJSON::ParserRuntime::LineCounter/;

    my $parser = $self->{parser};
    my $apparently = $parser->{offsetlinenum}
             - Config::File::RRJSON::ParserRuntime::_linecount(${$self->{text}})
             + 1;
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::ColCounter;
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::OffsetCounter;
...
sub ' . $namespace . '::' . $self->{"name"} .  '
{
    ...
    Config::File::RRJSON::ParserRuntime::_trace(q{Trying rule: [' . $self->{"name"} . ']},
    ...
    my $lastsep;
    my $current_match;
    my $expectation = new Config::File::RRJSON::ParserRuntime::Expectation(q{' . $self->expected() . '});
    $expectation->at($_[1]);
    '. ($parser->{_check}{thisoffset}?'
    my $thisoffset;
    tie $thisoffset, q{Config::File::RRJSON::ParserRuntime::OffsetCounter}, \$text, $thisparser;
    ':'') . ($parser->{_check}{prevoffset}?'
    my $prevoffset;
    tie $prevoffset, q{Config::File::RRJSON::ParserRuntime::OffsetCounter}, \$text, $thisparser, 1;
    ':'') . ($parser->{_check}{thiscolumn}?'
    my $thiscolumn;
    tie $thiscolumn, q{Config::File::RRJSON::ParserRuntime::ColCounter}, \$text, $thisparser;
    ':'') . ($parser->{_check}{prevcolumn}?'
    my $prevcolumn;
    tie $prevcolumn, q{Config::File::RRJSON::ParserRuntime::ColCounter}, \$text, $thisparser, 1;
    ':'') . ($parser->{_check}{prevline}?'
    my $prevline;
    tie $prevline, q{Config::File::RRJSON::ParserRuntime::LineCounter}, \$text, $thisparser, 1;
    ':'') . '
    my $thisline;
...
'
    unless ( $_matched || defined($score) )
    {
        ...
        Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' rule>>},
                     Config::File::RRJSON::ParserRuntime::_tracefirst($_[1]),
                     q{' . $self->{"name"} .'},
                     $tracelevel)
                    if defined $::RD_TRACE;
    ...
    if (defined $::RD_TRACE)
    {
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' rule<< (return value: [} .
                      $return . q{])}, "",
                      q{' . $self->{"name"} .'},
                      $tracelevel);
...
{
    my @items = @{$_[0]->{"items"}};
    if (@items==1 && ref($items[0]) =~ /\AConfig::File::RRJSON::ParserRuntime::Error/
        && $items[0]->{commitonly} )
    {
        Config::File::RRJSON::ParserRuntime::_warn(2,"Lone <error?> in production treated
                        as <error?> <reject>");
        Config::File::RRJSON::ParserRuntime::_hint("A production consisting of a single
                      ...
                      '<error?> <reject>' Config::File::RRJSON::ParserRuntime
                      supplied it for you.");
        push @{$_[0]->{items}},
            Config::File::RRJSON::ParserRuntime::UncondReject->new(0,0,'<reject>');
    }
    elsif (@items==1 && ($items[0]->describe||"") =~ /<rulevar|<autoscore/)
    {
        ...
        ( ref($items[0]) =~ /\AConfig::File::RRJSON::ParserRuntime::UncondReject/
        || ($items[0]->describe||"") =~ /<autoscore/
        ))
    {
        Config::File::RRJSON::ParserRuntime::_warn(1,"Optimizing away production: [". $_[0]->describe ."]");
        ...
        Config::File::RRJSON::ParserRuntime::_hint("The production starts with $what. That means that the
...
sub changesskip($)
{
    ...
    {
        if (ref($item) =~ /Config::File::RRJSON::ParserRuntime::(Action|Directive)/)
        {
...
sub addscore
{
    ...
    $self->additem(Config::File::RRJSON::ParserRuntime::Directive->new(
                  "local \$^W;
...
sub check_pending
{
    ...
    if ($self->{op})
    {
        ...
        {
        Config::File::RRJSON::ParserRuntime::_error("Incomplete <$next->{type}op:...>.", $line);
        Config::File::RRJSON::ParserRuntime::_hint(
...
sub enddirective
{
    ...
    unless ($self->{op})
    {
        Config::File::RRJSON::ParserRuntime::_error("Unmatched > found.", $line);
        Config::File::RRJSON::ParserRuntime::_hint(
    ...
    if ($op->{type} =~ /left|right/)
    {
        if ($span != 3)
        {
        Config::File::RRJSON::ParserRuntime::_error(
        ...
        {
        push @{$self->{items}},
            Config::File::RRJSON::ParserRuntime::Operator->new(
...
sub prevwasreturn
{
    ...
    push @{$self->{items}},
        Config::File::RRJSON::ParserRuntime::Result->new();
}
...
'           if $parser->{_check}{itempos};
    ...
    if ($parser->{_AUTOACTION} && defined($item) && !$item->isa("Config::File::RRJSON::ParserRuntime::Action"))
    {
        $code .= $parser->{_AUTOACTION}->code($namespace,$rule);
        Config::File::RRJSON::ParserRuntime::_warn(1,"Autogenerating action in rule
                       \"$rule->{name}\":
                        $parser->{_AUTOACTION}{code}")
        and
        Config::File::RRJSON::ParserRuntime::_hint("The \$::RD_AUTOACTION was defined,
                      so any production not ending in an
                      explicit action has the specified
                      \"auto-action\" automatically
                      appended.");
    }
    elsif ($parser->{_AUTOTREE} && defined($item) && !$item->isa("Config::File::RRJSON::ParserRuntime::Action"))
    {
        ...
        Config::File::RRJSON::ParserRuntime::_warn(1,"Autogenerating tree-building action in rule
                       \"$rule->{name}\"")
        and
        Config::File::RRJSON::ParserRuntime::_hint("The directive <autotree> was specified,
    ...
    $code .=
'
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' production: ['
                      . $self->describe . ']<<},
                      Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
...
' . ( $parser->{_check}{itempos} ? '
        if ( defined($_itempos) )
        {
            Config::File::RRJSON::ParserRuntime::Production::_update_itempos($_itempos, $itempos[ 1], undef, [qw(from)]);
            Config::File::RRJSON::ParserRuntime::Production::_update_itempos($_itempos, $itempos[-1], undef, [qw(to)]);
        }
' : '' ) . '
...
'
        ...
        {
            Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' action>> (return value: [undef])})
                    if defined $::RD_TRACE;
            last;
        }
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' action<< (return value: [}
                      . $_tok . q{])},
                      Config::File::RRJSON::ParserRuntime::_tracefirst($text))
                        if defined $::RD_TRACE;
        push @item, $_tok;
...
'
        ...
        Config::File::RRJSON::ParserRuntime::_trace(q{Trying directive: ['
                    . $self->describe . ']},
                    Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
        ...
        if (defined($_tok))
        {
            Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' directive<< (return value: [}
                        . $_tok . q{])},
                        Config::File::RRJSON::ParserRuntime::_tracefirst($text))
                            if defined $::RD_TRACE;
        }
        ...
        {
            Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' directive>>},
                        Config::File::RRJSON::ParserRuntime::_tracefirst($text))
                            if defined $::RD_TRACE;
        }
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::UncondReject;
...
'
        Config::File::RRJSON::ParserRuntime::_trace(q{>>Rejecting production<< (found '
                     . $self->describe . ')},
                     Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::Error;
...
sub code($$$$)
{
    ...
    {
        #WAS: $action .= "Config::File::RRJSON::ParserRuntime::_error(qq{$self->{msg}}" .  ',$thisline);';
    ...
    else      # GENERATE ERROR MESSAGE DURING PARSE
    {
        ...
        my $rule = $item[0];
           $rule =~ s/_/ /g;
        #WAS: Config::File::RRJSON::ParserRuntime::_error("Invalid $rule: " . $expectation->message() ,$thisline);
        push @{$thisparser->{errors}}, ["Invalid $rule: " . $expectation->message() ,$thisline];
        ';
    }

    my $dir =
          new Config::File::RRJSON::ParserRuntime::Directive('if (' .
...
sub new ($$$$$$)
{
    ...
    {
        Config::File::RRJSON::ParserRuntime::_warn(3, "Token pattern \"m$ldel$pattern$rdel$mod\"
...
my $code = '
        Config::File::RRJSON::ParserRuntime::_trace(q{Trying terminal: [' . $self->describe
                      . ']}, Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
                        if defined $::RD_TRACE;
        ...
        . ($check->{itempos}? 'do {'.Config::File::RRJSON::ParserRuntime::Production::incitempos().' 1} and ' : '')
        . '  $text =~ m' . $ldel . '\A(?:' . $self->{"pattern"} . ')' . $rdel . $mod . ')
        {
            '.($self->{"lookahead"} ? '$text = $_savetext;' : '$text = $lastsep . $text if defined $lastsep;') .
            ($check->{itempos} ? Config::File::RRJSON::ParserRuntime::Production::unincitempos() : '') . '
            $expectation->failed();
...
'
        Config::File::RRJSON::ParserRuntime::_trace(q{Trying subrule: [' . $self->{"subrule"} . ']},
                  Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
        ...
        {
            '.($self->{"lookahead"} ? '$text = $_savetext;' : '').'
            Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' subrule: ['
            . $self->{subrule} . ']>>},
                          Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
                          $tracelevel)
        ...
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' subrule: ['
...
sub new ($$$$$$$$$$)
{
    ...
    if ($lookahead)
    {
        if ($min>0)
        {
           return new Config::File::RRJSON::ParserRuntime::Subrule($subrule,$lookahead,$line,$desc,$matchrule,$argcode);
        }
        else
        {
            Config::File::RRJSON::ParserRuntime::_error("Not symbol (\"!\") before
                        \"$subrule\" doesn't make
                        sense.",$line);
            Config::File::RRJSON::ParserRuntime::_hint("Lookahead for negated optional
                       repetitions (such as
...
'
        Config::File::RRJSON::ParserRuntime::_trace(q{Trying repeated subrule: [' . $self->describe . ']},
                  Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
        ...
        . ')))
        {
            Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' repeated subrule: ['
            . $self->describe . ']>>},
                          Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
                          $tracelevel)
        ...
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' repeated subrule: ['
                    . $self->{subrule} . ']<< (}
                    . @$_tok . q{ times)},
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::Result;
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::Operator;
...
sub code($$$$)
{
    ...
    my $code = '
        Config::File::RRJSON::ParserRuntime::_trace(q{Trying operator: [' . $self->describe . ']},
                  Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
...
';
        ...
        $code .= '
          ...
          {
              $itemposfirst = Config::File::RRJSON::ParserRuntime::Production::_duplicate_itempos($_itempos);
          }
' if $check->{itempos};
        ...
        $code .= '
          ...
          while ($repcount < ' . $self->{max} . ')
          {
            ...
            ' . (ref($op) eq 'Config::File::RRJSON::ParserRuntime::Token'
...
' if $check->{itempos};
        ...
        $code .= '
            ...
            ' . (ref($op) eq 'Config::File::RRJSON::ParserRuntime::Token' ? 'do { push @item, $item{'.($self->{name}||$self->{hashname}).'}=$1; } if defined $1;' : "" ) . '
          }
          $text = $savetext;
          pop @item if $backtrack;
...
';
    ...
    $code .= '
          ...
          {
              Config::File::RRJSON::ParserRuntime::Production::_update_itempos(
...
';
    ...
    $code .= '
        ...
        {
            Config::File::RRJSON::ParserRuntime::_trace(q{<<'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self,1).' operator: ['
                          . $self->describe
                          . ']>>},
                          Config::File::RRJSON::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
        ...
        Config::File::RRJSON::ParserRuntime::_trace(q{>>'.Config::File::RRJSON::ParserRuntime::_matchtracemessage($self).' operator: ['
                      . $self->describe
                      . ']<< (return value: [}
                      . qq{@{$_tok||[]}} . q{]},
                      Config::File::RRJSON::ParserRuntime::_tracefirst($text),
...
package # this should not be indexed by CPAN
Config::File::RRJSON::ParserRuntime::Expectation;
...
sub _nextnamespace()
{
    return "Config::File::RRJSON::ParserRuntime::" . $nextnamespace++;
}
...
{
    my $class = ref($_[0]) || $_[0];
    local $Config::File::RRJSON::ParserRuntime::compiling = $_[2];
    my $name_space_name = defined $_[3]
        ? "Config::File::RRJSON::ParserRuntime::".$_[3]
        : _nextnamespace();
    my $self =
    {
        "rules"     => {},
        ...
        # wasn't present in some versions of Config::File::RRJSON::ParserRuntime used to
    ...
    if ($::RD_AUTOACTION) {
        ...
        $self->{_AUTOACTION}
            = new Config::File::RRJSON::ParserRuntime::Action($sourcecode,0,-1)
    }
...
sub Compile($$$$) {
    die "Compilation of Config::File::RRJSON::ParserRuntime grammars not yet implemented\n";
}

sub DESTROY {
    my ($self) = @_;
    my $namespace = $self->{namespace};
    $namespace =~ s/Config::File::RRJSON::ParserRuntime:://;
    if ($self->{_not_precompiled}) {
        ...
        # instead.
        {
            ...
            my $blank_sub = sub {};
            while (my ($name, $glob) = each %{"Config::File::RRJSON::ParserRuntime::$namespace\::"}) {
                *$glob = $blank_sub if defined &$glob;
            }
        }

        # Delete the namespace's stash
        delete $Config::File::RRJSON::ParserRuntime::{$namespace.'::'};
    }
}
...
sub _generate
{
    ...
    while (pos $grammar < length $grammar)
    {
            ...
            and do { ($code) = extract_codeblock($grammar); $code })
        {
            ...
            $item = new Config::File::RRJSON::ParserRuntime::Action($code,$lookahead,$line);
        ...
        elsif ($grammar =~ m/\G\s*<[^m]/gc)
        {
            ...
            elsif ($grammar =~ m/$UNCOMMITMK/gco)
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::Directive('$commit=0;1',
            ...
            elsif ($grammar =~ m/$QUOTELIKEMK/gco)
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::Directive(
                    'my ($match,@res);
            ...
            elsif ($grammar =~ m/$AUTOTREEMK/gco)
            {
                ...
                else
                {
                    ...
                    $self->{_AUTOTREE}{NODE}
                        = new Config::File::RRJSON::ParserRuntime::Action(q({bless \%item, ').$base.q('.$item[0]}),0,-1);
                    $self->{_AUTOTREE}{TERMINAL}
                        = new Config::File::RRJSON::ParserRuntime::Action(q({bless {__VALUE__=>$item[1]}, ').$base.q('.$item[0]}),0,-1);
                }
            }
            ...
            elsif ($grammar =~ m/$REJECTMK/gco)
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::UncondReject($lookahead,$line,"<reject>");
                $prod and $prod->additem($item)
                       ...
                       } )
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::UncondReject($lookahead,$line,$code);
                $prod and $prod->additem($item)
                      or  _no_rule($code,$line);
            }
        ...
        $self->{_AUTOACTION}
            = new Config::File::RRJSON::ParserRuntime::Action($code,0,-$line)
            }
            ...
            elsif ($grammar =~ m/$COMMITMK/gco)
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::Directive('$commit = 1',
        ...
        $self->{__TRACE__} = $1 ? $2+0 : 1;
            ...
            elsif ($grammar =~ m/$AUTOERRORMK/gco)
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::Error('',$lookahead,$1,$line);
                $prod and $prod->additem($item)
                    ...
                    $code })
            {
                ...
                $code =~ /\A\s*<error\??:(.*)>\Z/s;
                $item = new Config::File::RRJSON::ParserRuntime::Error($1,$lookahead,$commitonly,$line);
                $prod and $prod->additem($item)
                      or  _no_rule("$code",$line);
                $aftererror = !$commitonly;
            }
        ...
        elsif ($grammar =~ m/$RULE/gco)
        {
            ...
            if ($rulename =~ /Replace|Extend|Precompile|PrecompiledRuntime|Save/ )
            {
                _warn(2,"Rule \"$rulename\" hidden by method
                       Config::File::RRJSON::ParserRuntime::$rulename",$line)
                and
                _hint("The rule named \"$rulename\" cannot be directly
                       called through the Config::File::RRJSON::ParserRuntime object
                       for this grammar (although it may still
                       be used as a subrule of other rules).
                       It can't be directly called because
                       Config::File::RRJSON::ParserRuntime::$rulename is already defined (it
                       is the standard method of all
                       parsers).");
            }
            $rule = new Config::File::RRJSON::ParserRuntime::Rule($rulename,$self,$line,$replace);
            $prod->check_pending($line) if $prod;
            $prod = $rule->addprod( new Config::File::RRJSON::ParserRuntime::Production );
            $aftererror = 0;
        }
        elsif ($grammar =~ m/$UNCOMMITPROD/gco)
        {
            ...
            $prod->check_pending($line) if $prod;
            $prod = new Config::File::RRJSON::ParserRuntime::Production($line,1);
        ...
        elsif ($grammar =~ m/$ERRORPROD/gco)
        {
            ...
            _parseunneg("a new (error) production", $aftererror,
                    $lookahead,$line, substr($grammar, $-[0], $+[0] - $-[0]) ) or next;
            $prod->check_pending($line) if $prod;
            $prod = new Config::File::RRJSON::ParserRuntime::Production($line,0,1);
            $rule and $rule->addprod($prod)
                  or  _no_rule("<error>",$line);
            $aftererror = 0;
        }
        elsif ($grammar =~ m/$PROD/gco)
        {
            ...
            $rule
              ...
              and $prod = $rule->addprod(new Config::File::RRJSON::ParserRuntime::Production($line))
        ...
        elsif ($grammar =~ m/$LITERAL/gco)
        {
            ...
            $item = new Config::File::RRJSON::ParserRuntime::Literal($code,$lookahead,$line);
            $prod and $prod->additem($item)
        ...
        elsif ($grammar =~ m/$INTERPLIT/gco)
        {
            ...
            $item = new Config::File::RRJSON::ParserRuntime::InterpLit($1,$lookahead,$line);
            $prod and $prod->additem($item)
        ...
        elsif ($grammar =~ m/$TOKEN/gco)
        {
            ...
            $item = new Config::File::RRJSON::ParserRuntime::Token($1,'/',$3?$3:'',$lookahead,$line);
            $prod and $prod->additem($item)
...

        {
            ...
            $item = new Config::File::RRJSON::ParserRuntime::Token(@components[3,2,8],
                                 $lookahead,$line);
            $prod and $prod->additem($item)
        ...
        # EXTRACT TRAILING REPETITION SPECIFIER (IF ANY)
            ...
            if ($grammar =~ m/\G[(]/gc)
            {
                ...
                if ($grammar =~ m/$OPTIONAL/gco)
                {
                    ...
                    $item = new Config::File::RRJSON::ParserRuntime::Repetition($name,$1,0,1,
                                       $lookahead,$line,
                                       $self,
                                       $matchrule,
                                       $argcode);
                ...
                elsif ($grammar =~ m/$ANY/gco)
                {
                    ...
                    {
                        $item = new Config::File::RRJSON::ParserRuntime::Repetition($name,$1,0,$MAXREP,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$MANY/gco)
                {
                    ...
                    {
                        $item = new Config::File::RRJSON::ParserRuntime::Repetition($name,$1,1,$MAXREP,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$EXACTLY/gco)
                {
                    ...
                    {
                        $item = new Config::File::RRJSON::ParserRuntime::Repetition($name,$1,$1,$1,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$BETWEEN/gco)
                {
                    ...
                    {
                        $item = new Config::File::RRJSON::ParserRuntime::Repetition($name,"$1..$2",$1,$2,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
            ...
            else
            {
                ...
                $item = new Config::File::RRJSON::ParserRuntime::Subrule($name,
                                       $lookahead,
                                       $line,
                                       $desc,
                                       $matchrule,
    ...
    unless ($ERRORS or $isimplicit or $Config::File::RRJSON::ParserRuntime::compiling)
    {
        my $code = $self->_code();
        if (defined $::RD_TRACE)
        {
    ...
    # CHECK FOR UNDEFINED RULES
        ...
        foreach $call ( @{$rule->{"calls"}} )
        {
            if (!defined ${$rules}{$call}
              &&!defined &{"Config::File::RRJSON::ParserRuntime::$call"})
            {
...
. qq!
    \${"AUTOLOAD"} =~ s/^$self->{namespace}/Config::File::RRJSON::ParserRuntime/;
    goto &{\${"AUTOLOAD"}};
}
}

!;
    $code .= "push \@$self->{namespace}\::ISA, 'Config::File::RRJSON::ParserRuntime';";
    $self->{"startcode"} = '';
...
# keep the output in a form suitable for Config::File::RRJSON::ParserRuntime.
#
...
sub _parserepeat($$$$$$$$$)    # RETURNS A REF TO AN ARRAY OF MATCHES
{
    ...
    if (defined $itemposfirst)
    {
        Config::File::RRJSON::ParserRuntime::Production::_update_itempos($_itempos, $itemposfirst, undef, [qw(from)]);
    }
...
sub _parse($$$$)
{
    ...
    my $errorprefix = "Config::File::RRJSON::ParserRuntime";
    $errortext =~ s/\s+/ /g;
    _write_ERROR($errorprefix, $errortext);
}
